...
- ログアウト: プロフィール編集の「保存する」の下。`muted` の文字ボタン。確認ダイアログは出さない
...
- 試合入力のトビ・焼き鳥・その他ポイント・試合個別pt・手動ウマは、値が 0 のとき空欄。プレースホルダは `0`。空は 0 として計算する
...
フォーム内の参加者・ゲストは削除ではなく選択解除なので、行右端の「外す」を維持する。確認ダイアログは出さない。「外す」はボタン（親の作成／保存 form とは別の操作）。作成中は一覧から外す。編集ではその場で参加者行を外す（画面全体はリロードしない）。試合に出ている人も外せる。その人の試合結果と修正ポイントも消える。
...
**編集**（`/profile`）: 表示名、コメント、保存する。アイコンは表示のみ。「保存する」の下に `muted` の文字ボタン「ログアウト」（確認なし。ログインへ）。最下部に「アプリを退会する」
...
参加者・ゲストの行右端が「外す」（ボタン。確認は出さない）。編集ではその場で外す。試合に出ている人も外せる（その人の試合結果も消える）。
...
**ポイントの補正**: 縦＝参加者、横＝試合 pt（読み取り）＋補正（初期 1 列、＋で追加、最大 5）＋右端に差し引きの合計 pt。タイトルは列ヘッダー。数値欄のプレースホルダは `0`。保存する
