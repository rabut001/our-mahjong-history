...
| H | ルール項目の確定（第一級 vs フリー） | 第一級は 1-2 で確定（レートを含む）。計算式の細部は Phase 2〜4 | 1-2 / 1-4 |
...
- [x] エンティティ一覧と親子関係・多重度
- [x] 保存 vs 計算（ポイントは自動計算＋手入力を保存。順位は保存時計算。大会サマリーは 1-3）
...
- [x] 第一級項目（人数、持ち点・返し、ウマ一式、トビ有無、焼き鳥有無、レート）
