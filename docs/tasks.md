...
- [x] ダミーデータ（1 コミュニティ、大会複数、三麻/四麻混在）
...
見せ方（仮。2-4 で再確認可）: 1 画面スクロール。日付・ルールの下にメモ（コミュニティ詳細と同じく最大3行、空なら出さない）。見出しは「総合順位」（途中経過でも見るため）。総合順位は最終ptのみ。未出場は同じリストで順位を「-」。補正は総合順位タイトル行右の「ポイント補正」から別画面。その画面は縦＝利用者、横＝試合pt＋補正（初期1列、＋で追加、最大5）＋右端に差し引きの合計pt。保存は見せるだけ。試合一覧は `#n`（新しい試合が上）と順位・ポイント。追加は試合一覧タイトル行右の「追加」。各試合は右の「詳細」。参加者は全員チェックではなく、出る人だけ追加（多いときは名前検索）。大会編集のルールは追加・削除の見た目のみ（この画面上では消える。再読み込みで戻る。保存しない）。試合で使っているルールは削除ボタンを無効表示。
...
見せ方（仮）: コミュニティ詳細の下部に既定ルール一覧（追加 / 詳細）。大会は編集画面から詳細・追加。大会への追加は、コミュニティ既定からのコピー選択 → フォーム（値は複製。コピー後に大会用へ直せる）。いちから作成も可。既定が 0 件ならいちから作成のみ。フォームは 1 画面。人数で三麻/四麻を切り替え、ウマありのときだけ同着とウマpt、四麻のときだけウマ（2位⇔3位）。その他ポイントは見出し右の「追加」（最大5、未入力時は枠1つ、プレースホルダ「例：役満ご祝儀」）。大会ルールは試合で使っていれば閲覧のみ（新規登録へ案内）。未使用は編集・削除可。作成中の大会はコピー結果の見た目のみ（遷移なし）。保存しない。
