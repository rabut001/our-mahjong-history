...
UI 上の表記（アプリ名、ページタイトル、OG タイトル等）は **俺たちの雀歴** を用いる。リポジトリ名・パッケージ名等の識別子には **Our Mahjong History**（`our-mahjong-history`）を用いる。ドメインの「コミュニティ」は、UI では **麻雀グループ** と書く。
...
麻雀仲間のグループ（**コミュニティ**）で、麻雀大会および各試合（半荘）の結果を記録・共有する Web アプリケーション。**対局記録専用**であり、賭け麻雀や金銭のやり取りは扱わない。
...
- **コミュニティ**: 最上位のグループ。メンバー管理・ルール設定の単位。名称とコメント（説明）を持つ。UI では **麻雀グループ**
...
1. **PostgreSQL の関数**（SECURITY DEFINER）: 匿名化（表示名を「退会済みユーザ」、コメントと `avatar_url` を空）、`withdrawn_at`、`auth_user_id` を NULL、全コミュニティ離脱（最後の 1 人ならコミュニティ削除）
